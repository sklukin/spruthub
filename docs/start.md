...
- **Свой сервер или мини-ПК** (Raspberry Pi, Intel NUC, любой Linux-сервер)
- Docker и Docker Compose
- Сетевой доступ между SprutHub и сервером

### Почему свой сервер?

Все данные хранятся локально — графики и метрики **не зависят от облачных сервисов**:
- Работает без интернета
- Нет задержек при обращении к внешним API
- Полный контроль над данными
- Нет лимитов на объём хранения (только размер диска)
- Исторические данные доступны даже при отключении облака SprutHub
...
### Настройка батчинга (опционально)

```javascript
const ENABLE_BATCH_MODE = true;   // true = батчинг, false = мгновенная отправка
const BATCH_INTERVAL_MS = 10000;  // Интервал flush (10 секунд)
...
```javascript
const CRON_SCHEDULE = "0 0 * * * *";           // Отправка метрик (раз в час)
const REFRESH_INTERVAL = "0 * * * * *";         // Обновление списка устройств (раз в минуту)
const HEALTH_CHECK_SCHEDULE = "0 0,30 * * * *"; // Проверка доступности БД (раз в 30 мин)
```
...
**Батчинг** снижает нагрузку на хаб — вместо HTTP-запроса на каждое изменение датчика, метрики накапливаются в буфере и отправляются каждые 10 секунд.
...
- Данные собираются двумя способами:
  - **По триггеру с батчингом** — метрики накапливаются и отправляются каждые 10 секунд (снижает нагрузку на хаб)
  - **По cron** — периодически раз в час (для агрегированных отчётов, всегда батчом)
- **Timestamp** — время каждой метрики фиксируется в момент trigger, а не в момент отправки батча
